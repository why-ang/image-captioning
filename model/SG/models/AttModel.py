...
################################################################################################################
    ...
    def _sample_beam(self, fc_feats, att_feats,all_topic_word, att_masks=None, opt={}):
        ...
        for k in range(batch_size):
            ...
            self.done_beams[k] = self.beam_search(state, logprobs, tmp_fc_feats, tmp_att_feats, tmp_p_att_feats, tmp_mean_word, tmp_att_word, tmp_p_att_word,tmp_att_masks, opt=opt)
            seq[:, k] = self.done_beams[k][0]['seq'] # the first beam has highest cumulative score
            seqLogprobs[:, k] = self.done_beams[k][0]['logps']
        # return the samples and their log likelihoods
        return seq.transpose(0, 1), seqLogprobs.transpose(0, 1)

    def _sample(self, fc_feats, att_feats, all_topic_word, att_masks=None, opt={}):
        ...
        if beam_size > 1:
            return self._sample_beam(fc_feats, att_feats, all_topic_word,att_masks, opt)
...
class TopDownCore(nn.Module):
    def __init__(self, opt, use_maxout=False):
        ...
        self.v_lstm = nn.LSTMCell(opt.input_encoding_size + opt.rnn_size * 2, opt.rnn_size) # we, fc, h^2_t-1
        self.w_lstm = nn.LSTMCell(opt.input_encoding_size + opt.rnn_size * 2, opt.rnn_size) # we, fc, h^2_t-1
        
        self.lang_lstm = nn.LSTMCell(opt.rnn_size * 4, opt.rnn_size) # h^1_t, \hat v
        self.v_attention = Attention(opt)
        self.w_attention = Attention(opt)
        
        
        self.sigmoid = nn.Sigmoid()
        self.ff = nn.Linear(opt.rnn_size*4, opt.rnn_size)
