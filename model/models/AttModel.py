...
class TopDownCore(nn.Module):
    def __init__(self, opt, use_maxout=False):
        ...
        self.h2one = nn.Linear(opt.rnn_size, 1)
        self.v2one = nn.Linear(opt.input_encoding_size, 1)
    ...
    def forward(self, xt, fc_feats, att_feats, p_att_feats, state, att_masks=None):
        ...
        att_val = self.attention(h_att, att_feats, p_att_feats, att_masks)
        
        b = F.tanh(self.h2one(h_att)+self.v2one(fc_feats))
        
        b = 4 ** b 
        
        weight = F.softmax(att_val * b , dim=1)                             
        att_res = torch.bmm(weight.unsqueeze(1), att_feats).squeeze(1)
